...
from sklearn.tree import DecisionTreeClassifier as DTC1
...
for depth in depths:
	...
	for (train, test) in kfold.split(X_train):
		...
		X_train_CV_sck = X_train_sck.iloc[train, :]
		X_dev_CV_sck = X_train_sck.iloc[test, :]
		y_train_CV_sck = y_train_sck.iloc[train]
		y_dev_CV_sck = y_train_sck.iloc[test]
		
		# Train model with appropriate max_depth
		DT_Classifier_CV_gini = DecisionTreeClassifier(max_depth = depth, info_metric = 'Gini')
		clf = DTC1(max_depth = depth)
		...
		clf.fit(X_train_CV_sck, y_train_CV_sck)
		...
		y_train_preds_sck = clf.predict(X_train_CV_sck)
		...
		y_dev_preds_sck = clf.predict(X_dev_CV_sck)
		...
		train_acc_sck = accuracy_score(y_train_CV_sck, y_train_preds_sck)
		dev_acc_sck = accuracy_score(y_dev_CV_sck, y_dev_preds_sck)

		train_acc_iter_sck.append(train_acc_sck)
		dev_acc_iter_sck.append(dev_acc_sck)
		...
		print("Scikit Cross-Val Iter: {}, Cross-Val Train Accuracy: {}, Cross-Val Dev Accuracy: {}".format(idx, train_acc_sck, dev_acc_sck))
	...
	train_acc_CV_sck.append(np.mean(train_acc_iter_sck))
	dev_acc_CV_sck.append(np.mean(dev_acc_iter_sck))
...
plot_acc(depths = depths, train_acc = train_acc_CV_gini, dev_acc = dev_acc_CV_gini, title = 'gini', fname = 'plots/Xval_Gini_1.png')
plot_acc(depths = depths, train_acc = train_acc_CV_sck, dev_acc = dev_acc_CV_sck, title = 'scikit', fname = 'plots/Xval_scikit.png')

np.savetxt("preds/gini_accs_1_t.csv", train_acc_CV_gini, delimiter=",")
np.savetxt("preds/sck_accs_t.csv", train_acc_CV_sck, delimiter=",")

np.savetxt("preds/gini_accs_1_d.csv", dev_acc_CV_gini, delimiter=",")
np.savetxt("preds/sck_accs_d.csv", dev_acc_CV_sck, delimiter=",")
